...
test.describe('i18n - Пользовательский кабинет', () => {
	...
	test('Должен отображать LanguageSelector', async ({ page }) => {
		...
		const languageButton = page
			...
			.or(page.locator('button:has(svg[class*="globe"])'));
	...
	test('Должен переключаться между всеми 9 языками', async ({ page }) => {
		for (const lang of LANGUAGES) {
			...
			const languageOption = page
				.locator(`button:has-text("${lang.name}")`)
				.or(page.locator(`button:has-text("${lang.flag}")`))
				.first();
	...
	test('Казахский язык (kk) должен работать корректно', async ({ page }) => {
		...
		// Выбрать казахский
		const kazakhOption = page
			.locator('button:has-text("Қазақша")')
