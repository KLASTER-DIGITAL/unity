...
test.describe('i18n - Админ-панель', () => {
  test.beforeEach(async ({ page }) => {
    ...
    const languagesLink = page.locator('a:has-text("Languages")').or(
      page.locator('a:has-text("Языки")')
    ).first();
  ...
  test('Должен отображать список всех языков', async ({ page }) => {
    ...
    const languages = ['Russian', 'English', 'Spanish', 'German', 'French', 'Chinese', 'Japanese', 'Georgian', 'Kazakh'];
    
    for (const lang of languages) {
      const langElement = page.locator(`text=${lang}`).or(
        page.locator(`td:has-text("${lang}")`)
      );
  ...
  test('Должен показывать статистику переводов', async ({ page }) => {
    ...
    const statsTab = page.locator('button:has-text("Statistics")').or(
      page.locator('button:has-text("Статистика")')
    ).first();
    ...
    for (const code of languageCodes) {
      ...
      const statsElement = page.locator(`text=${code.toUpperCase()}`).or(
        page.locator(`td:has-text("${code}")`)
      );
  ...
  test('Должен открывать детальную страницу языка', async ({ page }) => {
    ...
    const kazakhRow = page.locator('tr:has-text("Kazakh")').or(
      page.locator('tr:has-text("Қазақша")')
    ).first();
    ...
    if (await kazakhRow.isVisible()) {
      ...
      const detailsVisible = await page.locator('h1:has-text("Kazakh")').or(
        page.locator('h2:has-text("Kazakh")')
  ...
  test('Должен показывать переводы для выбранного языка', async ({ page }) => {
    ...
    const translationsTab = page.locator('button:has-text("Translations")').or(
      page.locator('button:has-text("Переводы")')
    ).first();
    ...
    const translationsTable = page.locator('table').or(
  ...
  test('Должен фильтровать переводы по языку', async ({ page }) => {
    ...
    const languageFilter = page.locator('select').or(
