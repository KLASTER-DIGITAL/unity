...
import type { AnimatedViewProps, AnimatedPresenceProps, AnimationConfig, TransitionConfig } from './types';
...
async function initReanimated(): Promise<void> {
  ...
  try {
    ...
    throw new Error('react-native-reanimated is not available. Make sure react-native-reanimated is installed.');
  }
}
...
function convertToReanimatedStyle(config: AnimationConfig | undefined): any {
  ...
  if (config.x !== undefined || config.y !== undefined) {
    ...
    if (config.x !== undefined) {
      const xValue = typeof config.x === 'string'
        ? parseFloat(config.x)
        : config.x;
    ...
    if (config.y !== undefined) {
      const yValue = typeof config.y === 'string'
        ? parseFloat(config.y)
        : config.y;
...
function getAnimationFunction(transition?: TransitionConfig): (toValue: any, config?: any) => any {
  ...
  if (!transition || transition.type === 'spring') {
    ...
    return (toValue: any) => Reanimated!.withSpring(toValue, {
    ...
    return (toValue: any) => Reanimated!.withTiming(toValue, {
...
export const AnimatedView: React.FC<AnimatedViewProps> = ({
  ...
  useEffect(() => {
    initReanimated().then(() => setIsReady(true)).catch(console.error);
  }, []);
