...
export class NativeMediaAdapter implements MediaAdapter {
  ...
  private async init(): Promise<void> {
    ...
    if (typeof navigator === 'undefined' || navigator.product !== 'ReactNative') {
      console.warn('⚠️ [NativeMediaAdapter] Not in React Native environment, skipping initialization');
      return;
    }
  ...
  createCanvas(_width: number, _height: number): any {
    console.warn('⚠️ [NativeMediaAdapter] Canvas not supported in React Native. Use react-native-skia instead.');
    return null;
  }
