...
export function PushNotificationManager() {
	...
	const [selectedTemplate, setSelectedTemplate] =
		useState<PushTemplateType>("daily_reminder");
	...
	const loadVapidKeys = async () => {
		try {
			...
			const privateKeyRow = data?.find(
				(row) => row.key === "vapid_private_key",
			);
	...
	const loadStats = async () => {
		try {
			...
			const totalSent =
				historyData?.reduce((sum, row) => sum + (row.total_sent || 0), 0) || 0;
			const totalDelivered =
				historyData?.reduce(
					(sum, row) => sum + (row.total_delivered || 0),
					0,
				) || 0;
						...
						{!(vapidPublicKey && vapidPrivateKey) ? (
							<div className="space-y-4">
								<p className="text-muted-foreground text-sm dark:text-muted-foreground">
									VAPID keys не настроены. Сгенерируйте их для работы Web Push
									API.
								</p>
								<button
					...
					<div className="flex items-center gap-2">
						<input
							...
							onChange={(e) => {
								...
								if (e.target.checked) {
									const template = getPushTemplate(
										selectedTemplate,
										selectedLanguage,
									);
