		...
		<div className="w-full" data-testid="chart">
				...
				<table className="w-full border-collapse">
					<thead>
						<tr className="border-border border-b">
							...
							{numericKeys.map((key) => (
								<th
									className="p-2 text-right font-medium text-muted-foreground text-sm"
									key={key}
								>
									{key}
								</th>
			...
			<div className="mt-4 rounded-lg bg-muted/30 p-4">
				<p className="text-muted-foreground text-xs">
					⚠️ Временное отображение данных в виде таблицы. Графики будут
					восстановлены после исправления совместимости recharts с Vite.
				</p>
			</div>
		</div>
	);
