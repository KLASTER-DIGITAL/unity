...
const skeletonVariants = cva('animate-pulse rounded-card bg-accent', {
	variants: {
		variant: {
			default: 'bg-accent',
...
const Skeleton = React.forwardRef<HTMLDivElement, SkeletonProps>(
	(
		{ className, variant, size, width, height, duration = 2, shimmer = true, style, ...props },
		ref
	) => {
		const customStyle = {
...
const SkeletonAvatar = React.forwardRef<HTMLDivElement, Omit<SkeletonProps, 'variant'>>(
	({ className, size = 'default', ...props }, ref) => {
		...
		const avatarSize = avatarSizeMap[size as keyof typeof avatarSizeMap] || 'w-10 h-10';

		return <Skeleton ref={ref} variant="avatar" className={cn(avatarSize, className)} {...props} />;
	}
);
...
>(({ className, showImage = true, showHeader = true, showFooter = true, ...props }, ref) => (
	<div
		ref={ref}
		className={cn('rounded-card border bg-card p-0 overflow-hidden', className)}
		{...props}
	>
		{showImage && <Skeleton className="w-full h-48 rounded-none rounded-t-xl" />}
