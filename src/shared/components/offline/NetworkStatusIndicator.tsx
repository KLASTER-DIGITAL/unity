...
export const NetworkStatusIndicator = memo(function NetworkStatusIndicator() {
  ...
  const getStatusColor = (): string => {
    ...
    if (syncInProgress) return 'bg-yellow-500'; // 🟡 Syncing
  ...
  return (
    <span 
      className="absolute bottom-0 right-0 flex h-4 w-4 z-20"
    ...
    >
      ...
      {syncInProgress && (
        <span 
          className="animate-ping absolute inline-flex h-full w-full rounded-full bg-yellow-500 opacity-75"
          aria-hidden="true"
        />
      )}
      ...
      <span 
        className={`relative inline-flex rounded-full h-4 w-4 ${statusColor} ring-2 ring-white transition-colors duration-300`}
        aria-hidden="true"
      />
    </span>
  );
