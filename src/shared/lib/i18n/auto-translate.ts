...
export class AutoTranslationService {
	...
	static async translateMissingKeys(
		...
		const sourceTranslations =
			await AutoTranslationService.getTranslations(sourceLanguage);

		// 2. Определяем отсутствующие переводы
		const missingTranslations =
			await AutoTranslationService.getMissingTranslations(
				allKeys,
				targetLanguages,
			);
		...
		for (const targetLanguage of targetLanguages) {
			...
			console.log(
				`Translating ${missingKeys.length} keys to ${targetLanguage}`,
			);
			...
			for (let i = 0; i < batches.length; i++) {
				...
				if (i < batches.length - 1) {
					await AutoTranslationService.delay(
						AutoTranslationService.RETRY_DELAY,
					);
				}
			}
			...
			await AutoTranslationService.saveTranslations(
				targetLanguage,
				languageResults,
			);
		}
		...
		while (retryCount < AutoTranslationService.MAX_RETRIES) {
			try {
				...
				return keys.map((key) => ({
					...
					confidence: AutoTranslationService.calculateConfidence(
						translations[key],
						key,
					),
					needsReview: AutoTranslationService.needsReview(
						translations[key],
						key,
					),
				}));
				...
				console.error(
					`Translation batch error (attempt ${retryCount + 1}):`,
					error,
				);
				retryCount++;
				...
				if (retryCount < AutoTranslationService.MAX_RETRIES) {
					...
					console.error(
						`All retries failed for batch to ${targetLanguage}, using fallback`,
					);
					return keys.map((key) => ({
						key,
						originalText: sourceTranslations[key] || key,
...
- Preserve any emojis, special characters, or placeholders`;
	...
	private static async getMissingTranslations(
		...
		for (const language of targetLanguages) {
			try {
				const translations =
					await AutoTranslationService.getTranslations(language);
				missing[language] = allKeys.filter((key) => !translations[key]);
			} catch (error) {
				console.error(
					`Error checking missing translations for ${language}:`,
					error,
				);
