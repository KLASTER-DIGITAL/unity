...
export function getPushRecommendations(): string[] {
	...
	if (support.isSupported) {
		recommendations.push(
			"✅ Ваш браузер полностью поддерживает Push Notifications",
		);
		...
		if (support.browserInfo.os === "iOS") {
			recommendations.push(
				"💡 Для iOS рекомендуем использовать нативное приложение",
			);
			recommendations.push(
				"💡 Или используйте альтернативные методы уведомлений (email, SMS)",
			);
		} else if (support.browserInfo.name === "Safari") {
			recommendations.push("💡 Обновите Safari до версии 16+ (macOS Ventura)");
			recommendations.push(
				"💡 Или используйте Chrome/Firefox для полной поддержки",
			);
		} else {
