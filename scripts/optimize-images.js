...
function findImages() {
	...
	if (!fs.existsSync(assetsDir)) {
		console.log(
			`${colors.red}❌ Assets directory not found: ${assetsDir}${colors.reset}`,
		);
		return images;
	}
...
function analyzeImages() {
	...
	images.forEach((image) => {
		...
		console.log(
			`  ${color}${sizeStr.padStart(10)}${colors.reset} ${image.name}${warning}`,
		);
	});
	...
	if (largeImages > 0) {
		...
		console.log(
			`  Found ${largeImages} large image(s) that should be optimized:`,
		);
		console.log(`  
...
function createWebPVersions() {
	...
	const images = findImages().filter((img) =>
		[".png", ".jpg", ".jpeg"].includes(img.ext),
	);

	if (images.length === 0) {
		console.log(
			`${colors.yellow}No PNG/JPEG images found to convert${colors.reset}`,
		);
		return;
	}
	...
	try {
		...
		console.log(
			`${colors.red}❌ cwebp not found. Install WebP tools:${colors.reset}`,
		);
		console.log("  # macOS:");
  ...
  # Windows:`);
	...
	for (const image of images) {
		...
		if (fs.existsSync(webpPath)) {
			console.log(
				`  ${colors.yellow}⏭️  Skipping ${image.name} (WebP already exists)${colors.reset}`,
			);
			continue;
		}
		...
		try {
			...
			execSync(`cwebp -q 80 "${image.path}" -o "${webpPath}"`, {
				stdio: "ignore",
			});
			...
			console.log(
				`  ${colors.green}✅ ${image.name} → ${path.basename(webpPath)}${colors.reset}`,
			);
			console.log(
			...
			console.log(
				`  ${colors.red}❌ Failed to convert ${image.name}${colors.reset}`,
			);
		}
	}
	...
	if (converted > 0) {
		...
		console.log(
			`${colors.bright}Total savings: ${formatBytes(totalSavings)}${colors.reset}`,
		);
		console.log(`\n${colors.cyan}💡 Next steps:${colors.reset}`);
		console.log(
			"  1. Update your code to use WebP images with PNG/JPEG fallbacks",
		);
...
switch (command) {
	...
	case "help":
		...
		console.log(
			"  analyze  - Analyze images and show optimization recommendations (default)",
		);
	...
	default:
		...
		console.log(
			`Run 'node scripts/optimize-images.js help' for usage information.`,
		);
		process.exit(1);
}
