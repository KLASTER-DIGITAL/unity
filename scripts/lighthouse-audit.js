...
function installLighthouse() {
	...
	try {
		...
		console.log(
			`${colors.green}✅ Lighthouse installed successfully${colors.reset}`,
		);
		return true;
	} catch (error) {
...
function runLighthouseAudit(url = "http://localhost:3000") {
	console.log(
		`${colors.cyan}🔍 Running Lighthouse audit on ${url}...${colors.reset}`,
	);
...
function parseResults(jsonPath) {
	...
	return {
		...
		metrics: {
			...
			fid: lhr.audits["max-potential-fid"]
				? lhr.audits["max-potential-fid"].displayValue
				: "N/A",
...
function displayResults(results) {
	...
	console.log(
		`  SEO:             ${scoreColor(results.seo)}${results.seo}${colors.reset}`,
	);
	if (results.pwa) {
		console.log(
			`  PWA:             ${scoreColor(results.pwa)}${results.pwa}${colors.reset}`,
		);
	}
	...
	if (opportunities.length > 0) {
		opportunities.forEach(([key, audit]) => {
			...
			if (savings) {
				console.log(
					`  • ${audit.title}: ${Math.round(savings)}ms potential savings`,
				);
			}
		});
	} else {
		console.log(
			`  ${colors.green}✅ No major performance issues detected!${colors.reset}`,
		);
	}
}
...
async function runFullAudit() {
	console.log(
		`${colors.cyan}🚀 Starting Full Performance Audit${colors.reset}\n`,
	);

	// Проверяем Lighthouse
	if (!checkLighthouseInstalled()) {
		console.log(
			`${colors.yellow}Lighthouse not found. Installing...${colors.reset}`,
		);
	...
	try {
		...
		console.log(
			`${colors.blue}⏳ Waiting for server to be ready...${colors.reset}`,
		);
...
switch (command) {
	...
	case "help":
		console.log(
			`${colors.cyan}Lighthouse Performance Audit Tool${colors.reset}\n`,
		);
		console.log(
			"Usage: node scripts/lighthouse-audit.js [command] [options]\n",
		);
		console.log("Commands:");
		console.log(
			"  audit        - Run full audit on local dev server (default)",
		);
	...
	default:
		...
		console.log(
			`Run 'node scripts/lighthouse-audit.js help' for usage information.`,
		);
		process.exit(1);
}
