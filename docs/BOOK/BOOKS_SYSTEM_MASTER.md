...
- ✅ Редактор книги:
  ...
  - пересоздание PDF
- ✅ Версионирование (опционально, по умолчанию перезапись)
...
type StoryJson = {
  ...
  include_raw_entries: boolean;      // ✅ Включать ли сырые записи
};
```

### Структура книги (Premium)

Книга состоит из двух частей:
1.  **Часть 1: История (The Story)** — AI-сгенерированные главы, инсайты, выводы.
2.  **Часть 2: Хроника (The Chronicle)** — Полный список записей за период (как в Free версии), чтобы сохранить каждое слово.
```
 ...
 **Статус**: ✅ Работает (но есть ограничения)
 
 **Как работает**:
 1. Пользователь нажимает "Создать PDF" в редакторе.
 2. Браузер генерирует PDF Blob используя `@react-pdf/renderer`.
 3. Blob загружается в Supabase Storage через клиентский SDK.
 4. Ссылка сохраняется в `books_archive`.
 
 **Проблемы**:
 - Нагрузка на устройство пользователя (особенно на старых телефонах).
 - Проблемы с загрузкой шрифтов (CORS) в некоторых браузерах.
 - Vercel Serverless Function имеет лимит 10с (иногда не хватает для генерации).
 
 ### Планируемый переход: Puppeteer (Server-side)
 
 **Преимущества**:
 - Стабильность и независимость от устройства пользователя.
 - Полная поддержка Unicode и сложных макетов.
...
- ✅ Оригинальный PDF перезаписывается при сохранении (по желанию пользователя)
- ✅ Можно создать копию вручную ("Дублировать книгу")
- ✅ Статус "Готово" означает, что PDF сгенерирован и доступен для скачивания
...
### Новые реализации (Backend) ✅

- ✅ **FREE vs PREMIUM** - Реализовано разделение, `books-generate-free`
- ✅ **Puppeteer PDF** - Серверный рендеринг, поддержка Unicode
- ✅ **Snapshot Layer** - `monthly_snapshots` и агрегация данных
- ✅ **Entry Summaries** - Оптимизация токенов через саммари
- ✅ **Context Engine** - Поддержка персон и тегов


---

### 1. ❌ Дубликаты компонентов (Frontend)
...
### Фаза 1: Чистка Frontend и Синхронизация (P0)
...
2. **Интеграция Backend фич**:
   - [ ] Проверить вызов `books-render-puppeteer` с клиента
   - [ ] Проверить flow Free vs Premium на клиенте
...
- [Что нужно сделать (Анализ)](./BOOKS_SYSTEM_ANALYSIS_NEEDS.md)
- [Рекомендации](./BOOKS_SYSTEM_RECOMMENDATIONS.md)
- [Архив старых документов](./archive/)
