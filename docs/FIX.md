...
#### **Real-time синхронизация данных** (2025-11-21)
- **Добавлены real-time подписки**:
  - `app-shared/hooks/useUserData.ts`: подписка на изменения `entries` для автоматического пересчета статистики
  - `src/features/mobile/achievements/components/AchievementsScreen.tsx`: подписки на `entries` и `user_achievements`
  
- **Создан глобальный store**:
  - `src/shared/stores/useStatsStore.ts` (247 строк)
  - Использует Zustand для state management
  - Умный кэш с TTL 30 секунд
  - Автоматическая инвалидация через real-time подписки
  - API: `useUserStats(userId)` hook

- **Автоматическое тестирование**:
  - `scripts/test-realtime-sync.js` (212 строк)
  - Проверяет работу 3 подписок одновременно (Home, Achievements, Store)
  - Симулирует создание записи и проверяет получение real-time событий
  - Результат: ✅ 100% успех, все подписки работают

- **Документация**:
  - `docs/architecture/REALTIME_SYNC_IMPLEMENTATION.md` (259 строк)
    - Техническая документация архитектуры
    - Схема работы синхронизации
    - API и примеры использования
    - Преимущества и ограничения
  
  - `docs/changelog/2025-11-21_realtime_sync_fix.md` (310 строк)
    - Детальный отчет о проблеме и решении
    - Скриншоты до/после
    - Результаты тестирования
    - Чеклист завершения

- **Зависимости**:
  - Добавлен `zustand@^4.x.x` в package.json
  - 1343 пакета установлено через npm install

- **Изменения кода**:
  - 8 файлов изменено
  - +1136 строк добавлено
  - Нет удаленных строк (только расширение функционала)
...
- **Созданные файлы**:
  - `docs/i18n/I18N_AUDIT_REPORT_2025-11-20.md` (262 строки)
    - Полный технический отчет аудита системы переводов
    - Анализ покрытия для всех 9 языков
    - Список hardcoded текстов требующих миграции
    - Список 50 "мусорных" ключей в БД
    - Статус Platform Adapter (✅ COMPLETE)
    - Статус React Native адаптации (P1 - IMPORTANT)

  - `docs/i18n/I18N_CLEANUP_PLAN.md` (150 строк)
    - Детальный план очистки с чеклистом
    - Пошаговые инструкции для каждой задачи
    - Оценка времени выполнения (~40 минут)
    - Риски и митигация
    - Критерии успеха

  - `docs/i18n/I18N_AUDIT_SUMMARY_FOR_NON_PROGRAMMER.md` (150 строк)
    - Краткий отчет для не-программиста
    - Объяснение проблем простым языком
    - Рекомендации для менеджера проекта
    - Рекомендации для тестировщика
    - Рекомендации для разработчика

  - `supabase/migrations/20251120000001_cleanup_garbage_translation_keys.sql` (70 строк)
    - SQL скрипт для очистки БД от мусорных ключей
    - Удаление путей к файлам (../shadcn-io/*, @/components/*)
    - Удаление символов ( , -, :, *, /, a, T)
    - Удаление технических ключей (link, meta, script, ETag, webgl2, id)
    - Удаление статусов (syncing, offline, both)
    - Проверка результата (должно остаться ~580-600 ключей на язык)

- **Ключевые находки**:
  - ✅ 100% покрытие переводов для всех 9 языков
  - ❌ 6 hardcoded текстов в RecentEntriesFeed.tsx (строки 115, 146, 149, 185, 196, 214)
  - ❌ 50 "мусорных" ключей в БД (пути к файлам, символы, технические ключи)
  - ✅ Platform Adapter для i18n уже реализован (web + native)
  - ❌ 3 компонента без .native.tsx версий (AchievementHeader, ChatInputSection, MotivationCardsSection)

- **Приоритеты**:
  - P0 (КРИТИЧНО): Исправить hardcoded тексты + очистить БД (~20 минут)
  - P1 (ВАЖНО): Создать .native.tsx версии компонентов (4-6 часов)
  - P2 (МОЖНО ОТЛОЖИТЬ): Оптимизировать структуру ключей (2-3 часа)

- **Инструменты использованные**:
  - `codebase-retrieval` - анализ кодовой базы
  - `browser_navigate_Playwright` + `browser_snapshot_Playwright` - UI тестирование
  - `supabase` MCP - проверка БД
  - `view` - просмотр файлов
  - SQL queries - анализ дубликатов и мусорных ключей
...
**Root Cause Analysis**:
1. **Неправильное понимание таблицы motivation_cards**:
   - Таблица `motivation_cards` - это **ЛОГ просмотренных карточек**, а НЕ источник карточек для показа
   - Когда пользователь свайпает карточку → создается запись с `is_read = true` и `entry_id`
   - Это означает "пользователь просмотрел карточку из этой записи"

2. **home-screen-data Edge Function** делал неправильный запрос:
   - Запрашивал карточки из таблицы `motivation_cards` с фильтром `is_read = false`
   - Но в таблице ТОЛЬКО просмотренные карточки (is_read = true)!
   - Результат: пустой массив карточек

3. **Правильная логика** (motivations Edge Function):
   - Генерирует карточки **динамически** из записей (entries)
   - Берет последние 10 записей за 48 часов
   - Фильтрует непросмотренные (проверяет `motivation_cards.entry_id`)
   - Добавляет шаблонные карточки если < 3
   - **НЕ сохраняет** карточки в БД, только возвращает их

**Исправление**:
- `supabase/functions/home-screen-data/index.ts` (строки 61-128)
  - Удален неправильный запрос к таблице `motivation_cards`
  - Добавлен вызов `motivations` Edge Function для генерации карточек
  - Карточки теперь генерируются динамически из записей
  - Edge Function задеплоена (version 4)

**Логика обновления карточек**:
- Карточки обновляются **каждый раз** при открытии главного экрана
- Берутся последние 10 записей за 48 часов
- Фильтруются непросмотренные (не в `motivation_cards.entry_id`)
- Если < 3 карточек → добавляются шаблонные мотивационные сообщения
- Каждая карточка получает уникальный градиент на основе индекса

**Тестирование**:
- ✅ Build успешен (npm run build)
- ✅ Edge Function задеплоена (version 4)
- ✅ Логика проверена через codebase-retrieval

**Затронутые пользователи**: rustam@leadshunter.biz и все пользователи после деплоя

---

### 📚 Документация системы карточек

**Файл**: `docs/architecture/MOTIVATION_CARDS_SYSTEM.md`

**Содержание**:
1. Архитектура системы (Frontend → Edge Functions → Database)
2. Алгоритм генерации карточек (motivations Edge Function)
3. Источники данных (entries + motivation_cards)
4. Логика фильтрации просмотренных
5. Свайп и отметка "просмотрено"
6. Обновление карточек (автоматически + pull-to-refresh)
7. Шаблонные карточки (fallback когда < 3 карточек)
8. Push-уведомления (Database Webhooks + Cron Jobs)
9. Сценарии работы:
   - Новый пользователь (первый день)
   - Активный пользователь (7 день)
   - Пользователь с перерывом (возвращается через 3 дня)
10. Критические проблемы и решения:
    - Архитектурная ошибка (ИСПРАВЛЕНО)
    - 404 ошибки для CSS файлов (Service Worker автообновление)
    - Временное окно 48 часов vs ожидания пользователя

**Анализ push-уведомлений**:
- Карточки НЕ отправляют уведомления напрямую
- Уведомления отправляются при создании записей (Database Webhooks)
- 3 типа уведомлений:
  1. При создании записи (Realtime) - "✅ Запись сохранена!"
  2. При готовности AI-анализа (Realtime) - "🤖 AI-анализ готов!"
  3. Ежедневное напоминание (Scheduled, 09:00) - AI-персонализированное
...
- Обновлены классы с `bg-gradient-to-*` на `bg-linear-to-*` (Tailwind CSS v4)
- Файлы:
  - `src/features/mobile/home/components/AllCardsViewedModal.tsx` (3 градиента)
  - `src/features/mobile/home/components/achievement/SwipeCard.tsx` (1 градиент)
