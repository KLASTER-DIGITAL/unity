...
export const LanguagesTab: React.FC = () => {
		...
		try {
			...
			if (response.ok) {
				setTranslations((prev) =>
					prev.map((t) =>
						t.key === editingTranslation.key &&
						t.language === editingTranslation.language
							? { ...t, value: editValue }
							: t,
					),
				);
	...
	const getTranslationProgress = (languageCode: string) => {
		const totalKeys = 50;
		const translatedKeys = translations.filter(
											...
											<div className="admin-space-y-3">
												...
												<div className="admin-w-full admin-bg-gray-200 admin-rounded-full admin-h-2">
													<div
														...
														style={{
															width: `${getTranslationProgress(language.code)}%`,
														}}
													/>
												</div>
								...
								>
									<div className="admin-flex admin-items-start admin-justify-between admin-mb-3">
										<div className="admin-flex-1">
											...
											<div className="admin-text-gray-600 admin-text-xs">
												Ключ перевода
											</div>
										</div>
