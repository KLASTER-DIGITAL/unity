...
Deno.serve(async (req) => {
	...
	try {
		...
		if (path.length === 2 && /^[a-z]{2}$/.test(path) && req.method === "GET") {
			...
			const translationsObj = (translations || []).reduce(
				(acc: any, t: any) => {
			...
			console.log(
				`Translations found for ${langCode}:`,
		...
		switch (path) {
				...
				if (req.method === "GET") {
					...
					languages?.forEach((lang) => {
						const langTranslations =
							translations?.filter((t) => t.lang_code === lang.code) || [];
						translatedKeys[lang.code] = langTranslations.length;

						const translatedKeysForLang = langTranslations.map(
							(t) => t.translation_key,
						);
						const missingKeys =
							baseTranslations
								?.filter(
									(bt) => !translatedKeysForLang.includes(bt.translation_key),
								)
			...
			case "keys": {
				if (req.method === "GET") {
					...
					const keys = data
						? [...new Set(data.map((t) => t.translation_key))]
						: [];
			...
			case "missing": {
				if (req.method === "POST") {
					const { key, language, context, userAgent, timestamp } =
						await req.json();
