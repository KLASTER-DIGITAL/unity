...
Deno.serve(async (req) => {
	...
	try {
		...
		const relevantParts = pathParts.filter(
			(p) => !["functions", "v1", "admin-api"].includes(p),
		);
		// Remove 'admin' prefix if present (for /admin/stats -> /stats)
		const endpoint =
			relevantParts.filter((p) => p !== "admin").join("/") || "stats";
		...
		if (endpoint === "users" && req.method === "GET") {
			...
			const { data: usersRaw, error } = await supabaseAdmin.rpc(
			...
			if (error) {
				...
				console.warn(
					"⚠️ RPC function get_users_with_stats not found, using fallback",
				);
