...
Deno.serve(async (req: Request): Promise<Response> => {
	...
	try {
		...
		if (method === "POST" && url.pathname.includes("/upload")) {
			...
			console.log(
				`[MEDIA-UPLOAD v1] 📤 Uploading media for user ${userId}: ${fileName}`,
			);
			...
			console.log(
				`[MEDIA-UPLOAD v1] 📊 Base64 data length: ${base64Data.length} chars`,
			);
			...
			console.log(
				"[MEDIA-UPLOAD v1] ⬆️ Step 1: Uploading main file to Storage...",
			);
			...
			console.log(
				"[MEDIA-UPLOAD v1] ✅ Step 1 complete: Main file uploaded to Storage",
			);
			...
			if (thumbnail) {
				...
				const thumbnailBase64 = thumbnail.includes(",")
					? thumbnail.split(",")[1]
					: thumbnail;
			...
			if (signedUrlResponse.ok) {
				...
				console.log(
					`[MEDIA-UPLOAD v1] 🔗 Using fallback public URL: ${signedUrl}`,
				);
			}

			// ШАГ 3: Сохраняем метаданные в PostgreSQL
			console.log(
				"[MEDIA-UPLOAD v1] 💾 Step 3: Saving metadata to database...",
			);
