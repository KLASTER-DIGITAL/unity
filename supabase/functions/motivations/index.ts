	...
	const defaults: Record<string, any[]> = {
		...
		en: [
			...
			},
			{
				...
				description:
					"You don't have to write a lot — one phrase can change your view of the day.",
				gradient: "from-[#ff6b9d] to-[#c471ed]",
				isMarked: false,
				isDefault: true,
				sentiment: "calm",
...
async function handleRequest(req: Request): Promise<Response> {
	...
	try {
		...
		if (method === "GET" && url.pathname.startsWith("/motivations/cards/")) {
			...
			console.log(
				`[MOTIVATIONS v9] Found ${recentEntries.length} recent entries`,
			);
			...
			if (!viewedResponse.ok) {
				throw new Error(
					`Failed to fetch viewed cards: ${viewedResponse.status}`,
				);
			}
			...
			const unviewedEntries = recentEntries.filter(
				(entry: any) => !viewedIds.includes(entry.id),
			);
			console.log(
				`[MOTIVATIONS v9] Unviewed entries: ${unviewedEntries.length}`,
			);
		...
		if (method === "POST" && url.pathname === "/motivations/mark-read") {
			...
			console.log(
				`[MOTIVATIONS v9] Marking card ${cardId} as read for user ${userId}`,
			);
