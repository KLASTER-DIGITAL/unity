...
Deno.serve(async (req) => {
	...
	try {
		...
		if (req.method === "GET" && path.endsWith("/languages")) {
			...
			// Get unique keys
			const uniqueKeys = [
				...new Set(allTranslations.map((t) => t.translation_key)),
			];
			const totalKeys = uniqueKeys.length;

			// Calculate statistics for each language
			const languagesWithStats = languages.map((lang) => {
				const translationCount = allTranslations.filter(
					(t) => t.lang_code === lang.code,
				).length;
				const progress =
					totalKeys > 0 ? Math.round((translationCount / totalKeys) * 100) : 0;
